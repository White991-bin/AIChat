...
// 验证配置参数
bool validateConfig(ai_chat_server::ChatServerConfig& config) {
    // 验证温度值
    ...
    if (config.max_tokens <= 0) {
        ERROR("错误: 最大token数必须为正数,当前值: {}", config.max_tokens);
        return false;
    }

    // 验证至少有一个API密钥不为空
    if (config.DeepSeekAPIkey.empty() && config.GeminiAPIkey.empty()) {
        ERROR("错误: 至少需要提供一个有效的API密钥或Ollama模型配置");
        return false;
    }

    // 验证Ollama配置参数
    if (!config.ollama_ModelName.empty()) {
        if (config.ollama_ModelDesc.empty() || config.ollama_Endpoint.empty()) {    
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // 从命令行参数获取Ollama配置
        config.ollama_ModelName = FLAGS_ollama_model_name;
        config.ollama_ModelDesc = FLAGS_ollama_model_desc;
        config.ollama_Endpoint = FLAGS_ollama_endpoint;
        ...
        INFO("  最大Token: {}", config.max_tokens);
        INFO("  DeepSeek API Key: {}", (config.DeepSeekAPIkey.empty() ? "未设置" : "已设置"));
        INFO("  Gemini API Key: {}", (config.GeminiAPIkey.empty() ? "未设置" : "已设置"));
        INFO("  Ollama 模型: {}", (config.ollama_ModelName.empty() ? "未设置" : config.ollama_ModelName));
        INFO("  Ollama 模型描述: {}", (config.ollama_ModelDesc.empty() ? "未设置" : config.ollama_ModelDesc));
        INFO("  Ollama 端点: {}", (config.ollama_Endpoint.empty() ? "未设置" : config.ollama_Endpoint));    
